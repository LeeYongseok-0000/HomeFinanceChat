...
public class CustomFileUtil {
  ...
  public ResponseEntity<Resource> getFile(String filename) { 
    ...
    log.info("### getFile 요청 - 파일명: {}", filename);
    ...
    if (!Files.exists(filePath)) {
        log.warn("### getFile 경고 - 파일이 존재하지 않음: {}", filePath);
    ...
    try {
        ...
        log.warn("### 파일 크기 확인 실패: {}", e.getMessage());
    }
    ...
    try {
        ...
        headers.add("Content-Disposition", "inline; filename=\"" + decodedFileName + "\"");
        headers.add("Cache-Control", "max-age=31536000"); // 1년 캐시
        
        log.info("### Content-Type 설정: {}", contentType);
    } catch (IOException e) {
        log.error("### getFile 에러 - Content-Type 결정 중 오류:", e);
        return ResponseEntity.internalServerError().build();
    }
    
    log.info("### getFile 성공 - 파일 전송: {}, Content-Type: {}", filename, headers.getFirst("Content-Type"));
