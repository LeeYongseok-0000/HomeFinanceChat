...
// 게시글 생성 - "Q&A" 카테고리로 고정
export const createRealEstateQna = async (board) => {
...
// 게시글 수정
export const updateRealEstateQna = async (formData) => {
  try {
    ...
    console.log("=== updateRealEstateQna API 호출 ===");
    ...
    console.log("updateRealEstateQna 응답:", res);
    return res.data;
  } catch (error) {
    console.error("updateRealEstateQna API 에러:", error);
    if (error.response) {
...
// 게시글 조회
export const getRealEstateQna = async (bno) => {
  const res = await axios.get(`${boardHost}/read/${bno}`);
  return res.data;
};

// 게시글 목록 조회 - "Q&A" 카테고리로 고정
export const getRealEstateQnas = async (params = {}) => {
  try {
    const { page = 0, size = 10, searchTerm = "" } = params;

    // category를 "부동산 Q&A"로 고정
    let url = `${boardHost}/list?category=${encodeURIComponent("부동산 Q&A")}`;
    ...
    console.log("getRealEstateQnas API 호출 URL:", url);
    const res = await axios.get(url);
    console.log("getRealEstateQnas 원본 응답:", res);
    console.log("getRealEstateQnas 응답 데이터 타입:", typeof res.data);
    console.log("getRealEstateQnas 응답 데이터:", res.data);
    ...
    console.error("부동산 Q&A 목록 조회 실패:", error.message);
...
// 게시글 삭제
export const deleteRealEstateQna = async (bno) => {
  try {
    console.log("부동산 Q&A 삭제 요청 시작 - 게시글 ID:", bno);
    const res = await axios.delete(`${boardHost}/remove/${bno}`);
    console.log("부동산 Q&A 삭제 응답:", res);
    return res.data;
  } catch (error) {
    console.error("부동산 Q&A 삭제 실패:", error);
