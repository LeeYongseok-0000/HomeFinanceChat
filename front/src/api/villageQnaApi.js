...
// 게시글 생성 - "우리마을" 카테고리로 고정
export const createVillageQna = async (board) => {
...
// 게시글 수정
export const updateVillageQna = async (formData) => {
  try {
    ...
    console.log("=== updateVillageQna API 호출 ===");
    ...
    console.log("updateVillageQna 응답:", res);
    return res.data;
  } catch (error) {
    console.error("updateVillageQna API 에러:", error);
    if (error.response) {
...
// 게시글 조회
export const getVillageQna = async (bno) => {
  const res = await axios.get(`${boardHost}/read/${bno}`);
  return res.data;
};

// 게시글 목록 조회 - "우리마을" 카테고리로 고정
export const getVillageQnas = async (params = {}) => {
  try {
    const { page = 0, size = 10, searchTerm = "" } = params;

    // category를 "마을 Q&A"로 고정
    let url = `${boardHost}/list?category=${encodeURIComponent("마을 Q&A")}`;
    ...
    console.log("getVillageQnas API 호출 URL:", url);
    const res = await axios.get(url);
    console.log("getVillageQnas 원본 응답:", res);
    console.log("getVillageQnas 응답 데이터 타입:", typeof res.data);
    console.log("getVillageQnas 응답 데이터:", res.data);
    ...
    console.error("우리 마을 Q&A 목록 조회 실패:", error.message);
...
// 게시글 삭제
export const deleteVillageQna = async (bno) => {
  try {
    console.log("우리 마을 Q&A 삭제 요청 시작 - 게시글 ID:", bno);
    const res = await axios.delete(`${boardHost}/remove/${bno}`);
    console.log("우리 마을 Q&A 삭제 응답:", res);
    return res.data;
  } catch (error) {
    console.error("우리 마을 Q&A 삭제 실패:", error);
